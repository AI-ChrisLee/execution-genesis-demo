# The deck (the MEP: beats 1 to 3, and the plan's lists)
...
no `confirmed` stamp still carries it); the demo is read off it, never asked. A hybrid model
line takes its first word.
...
Day one is the plugin installed (once per laptop), the 7 answers, the direction, the build,
the 3 captures. Day two is the 9 boxes, the deck, the look, the fixes, one rehearsal of the
screen share.

**The plugin run, exactly.** Project root: `squad/mep/<name>/`, whichever name beat 0
resolved (the person, the company, or the buyer type).
...
- Phase 1, the brief. Fill `client.md` from the offer document and `notes.md`:
  ...
  `UNKNOWN` and the page carries nothing there. On that yes, append `answers yes <date>` as
  the last line of `client.md`; that line, not the file existing, is what says the founder
  saw the answers. Question 6 (3 sites they like, 1 they hate) is not asked. Answer it from
  the trade's `Style Shortlist` in the plugin's `trades.csv` and the `Do Not Use For` column
  in its `styles.csv`: the shortlist is the liked direction, the do-not-use line is the hated
  one. No site is browsed, and nothing about the buyer is read outside their folder.
...
only, so the second and third captures go through a throwaway wrapper page in the system temp
folder (deleted after) that frames `index.html` scrolled to the section: a 1920x1080 body,
`overflow:hidden`, holding `<iframe src="file://<folder>/index.html" style="width:1920px;
height:6000px;border:0;margin-top:-<offset>px">`, the offset being the pixel that section starts
at, read off the page (1080 and 2160 when nothing better is known; check each capture once and
move the offset when it cut a section in half). Not found: the deck still gets built, its 3 demo
slides carry a labeled hole each, and the founder gets 3 lines: open `index.html` in the browser
at full width, take 3 screenshots (the top, the middle, the money action), and save them in the
folder as `shot-1.png`, `shot-2.png`, `shot-3.png`. The deck fills in the moment the files land.
...
Day one is the paste, the piece. Day two is the read-aloud in their voice, the deck, the look.
...
structure shown 3 ways; on disk it is a CSV the founder can drop into their own Notion in a
minute (Settings, Import, CSV, or `/csv` on a page, then map each column to a property).
...
Day one is the columns, the rows, the CSV. Day two is the lit row, the line, the deck, the look.

**The file.** `structure.csv`: the header row is the columns, then the rows, plain CSV. No
Notion connector, no one-page diagnosis, no `notion.md`.
...
"roadmap" screens, a `prototype/` folder.

Day one is screens one and two. Day two is screen three, the deck, the look.
...
<head>
<meta charset="utf-8">
<title>MEP · <buyer name></title>
<style>
/* deck-cage.css, whole, here */
:root{--accent:<the roots file's accent color, or #146ef5>}
</style>
</head>
<body>
<div class="deck">
...
offer document, the row, or the founder's yes did not give.
...
Look, in this order, and use the first one that exists; say in one line which one, or that
none was found.
...
offer document's WHO line into `squad/mep/<buyer-type>/`, the same spine, with every
buyer-specific noun a marked blank in square brackets: `[their name]`, `[their town]`, `[their
review count]`. Slide 2 is the WHO line's problem in the offer document's words, labeled
`(offer document · <date>)`, no quote marks. Slide 3 is dropped. The plan's last line before
`go` says: rebuild under a name the day the first call books. Then `/mep <name>` builds the
named one; the buyer-type folder stays as it is.

Beat 4's line names nobody on this path, because every buyer-specific noun is a blank. Print
instead: `It is built on [their name] and [their town]: <n> marked blanks, on <n> slides.`
The gate under it reads: this one is a shape, not a deck. Say go and I file it; the day your
first call books, run `/mep <name>` and it gets built for a person.
...
- **The site.** The top slide comes off the row, with the review count and rating copied off
  Maps at the founder's yes. The hero has no customer words to use, so it carries the offer's
  result. The money action is unchanged.
...
The deck's Inter and the plugin's 16 pairings are Latin Google Fonts. A buyer whose market
writes in Korean, Japanese, Thai, Arabic or another non-Latin script gets the system font
there, which the plugin's box 2 marks open on the site. Build anyway. `plan.md` carries one
LANGUAGE line naming the gap, and the look at beat 4 says which box it opens.
